...
if (!defined('CONF_SCHEDULER_VERSION'))
    define('CONF_SCHEDULER_VERSION', '2.4.9');
...
class Conference_Scheduler {
  ...
  function get_session_workshops($session, $atts = array()) {
    ...
    return apply_filters('conf_scheduler_session_workshops', $workshops, $session, $atts);
  }
   ...
   */
	function render_schedule($atts) {
      ...
      $options = array('<optgroup label="'.__('Type to Search', 'conf-scheduler').'"></optgroup>');
      ...
      if ($themes) {
        $theme_options = '<optgroup label="'.__('Themes', 'conf-scheduler').'">';
      ...
      if ($keywords) {
        $keyword_options = '<optgroup label="'.__('Keywords', 'conf-scheduler').'">';
	...
	function render_session($session_tree, $atts = null) {
	  ...
	  foreach ($session_tree as $session_id => $session) {
        // skip if empty session in timeline mode
        if ($this->option('view_mode') == 'timeline' && $session->count == 0 && !$session->description) continue;
    ...
    <div id="conf-sidebar-wrap" style="display: none;">
      <div class="conf-sidebar">
            ...
            <ul>
              ...
              <li><?php printf( __('Multi-lingual integration with %s and %s','conf-scheduler'), '<a href="https://wpml.org/">WPML</a>', '<a href="https://wordpress.org/plugins/polylang/">Polylang</a>');?></li>
